...
class ShopCategoryData extends PolymerElement {
  ...
  _fetchItems(category, attempts) {
    this._setFailure(false); // Only fetch the items of a category if it has not been previously set.
  ...
  _getResource(rq, attempts) {
    ...
    xhr.addEventListener('error', e => {
      ...
      if (attempts > 1) {
        this._getResourceDebouncer = Debouncer.debounce(this._getResourceDebouncer, timeOut.after(200), this._getResource.bind(this, rq, attempts - 1));
      } else {
